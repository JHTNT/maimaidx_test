...
const verstr_list = ['ジングル','真','超','檄','橙','暁','桃','櫻','紫','菫','白','雪','輝','DX']
...
const get_music_lamp_data = (md) =>
{
	...
	for(let i=0; i<tmp.length; i++)
	{
		switch(tmp[i])
		{
			...
			case "sssp": rank=9; break; // SSS+ (100.5%)
			case "sss": rank=8; break; // SSS  (100.0%)
			case "ssp": rank=7; break;
			case "ss": rank=6; break;
			case "sp": rank=5; break;
			case "s": rank=4; break;
			case "aaa": rank=3; break;
			case "aa": rank=2; break;
			case "a": rank=1; break;
			default:
				break;
		}
...
const data2analysis_callback = (dlist, tlist, r_dlist, r_tlist, ba_data, ad_data, ex_data, ma_data, re_data) =>
{
	const sub = (lampdata) =>	// FDX|AP|FC|rank => FDX|AP|SSS|FC に変更
		{return (lampdata & 0xFFFF0000) | (((lampdata & 0xF) >= 8)?0x100:0) | ((lampdata & 0xFF00) >> 8); }
	...
	return new Promise((resolve) =>
	{
		...
		while(tlist[0] != undefined)
		{
			...
			stdclearcount[0] -= ((ba_data[0]&0xF)>0 && ver < 13)?1:0;
			dxclearcount[0] -= ((ba_data[0]&0xF)>0 && ver >= 13)?1:0;
			ad_list[ver] -= sub(ad_data[0]);
			stdclearcount[1] -= ((ad_data[0]&0xF)>0 && ver < 13)?1:0;
			dxclearcount[1] -= ((ad_data[0]&0xF)>0 && ver >= 13)?1:0;
			ex_list[ver] -= sub(ex_data[0]);
			stdclearcount[2] -= ((ex_data[0]&0xF)>0 && ver < 13)?1:0;
			dxclearcount[2] -= ((ex_data[0]&0xF)>0 && ver >= 13)?1:0;
			ma_list[ver] -= sub(ma_data[0]);
			stdclearcount[3] -= ((ma_data[0]&0xF)>0 && ver < 13)?1:0;
			dxclearcount[3] -= ((ma_data[0]&0xF)>0 && ver >= 13)?1:0;
			compdata[ver].push([
			...
			if(tmpmd.dx == r_dlist[0] && tmpmd.n == r_tlist[0])
			{
				...
				stdclearcount[4] -= ((re_data[0]&0xF)>0 && ver < 13)?1:0;
				dxclearcount[4] -= ((re_data[0]&0xF)>0 && ver >= 13)?1:0;
...
const print_result_sub_print_header = (title) =>
{
	...
	rslt_str += "<title>" + title + " | 新・CYCLES FUNの寝言</title>\n";
    	rslt_str += "<link rel='stylesheet' media='all' type='text/css' href='https://raw.githubusercontent.com/JHTNT/maimaidx_test/master/css/maidx_rating.css'>\n";
 	rslt_str += "<link rel='stylesheet' media='all' type='text/css' href='https://raw.githubusercontent.com/JHTNT/maimaidx_test/master/css/display.css'>\n";
 	rslt_str += "<link rel='stylesheet' media='all' type='text/css' href='https://raw.githubusercontent.com/JHTNT/maimaidx_test/master/css/result.css'>\n";
	...
	rslt_str += '<script src="https://raw.githubusercontent.com/JHTNT/maimaidx_test/master/scripts_maimai/maidx_comp_sub.js"></script>\n';
 	rslt_str += "</head>\n";
...
const print_lamp2str_comp = (lamp, classname)=>
{
	...
	const ranknum = lamp & 0xF
	
	let str = '';
	str += '<td class=' + classname + '>' + (((lamp & 0x01000000) != 0)?'舞':'　') + '</td>';
	
	if((lamp & 0x00010000) != 0)
		str += '<td colspan=2 class=' + classname + '>神</td>';
	else
	{
		str += '<td class=' + classname + '>' + ((ranknum >= 8)?'将':'　') + '</td>';
		str += '<td class=' + classname + '>' + (((lamp & 0x00000100) != 0)?'極':'　') + '</td>';
	}
	
	str += '<td class=' + classname + '>' + ((disp_rank)?(rankname[ranknum]):(ranknum>0)?'Clr':'　') + '</td>';
	return str;
}
...
const print_result = (ba, ad, ex, ma, re, name, plate, musicdata, re_musicdata, maicompdata, stdcompdata, dxcompdata, starttime) =>
{
...
if(location.href.slice(0,41) == mainet_dom+"friend/" && location.href.length > 41)
{
	...
	.then((x)=>{
		print_result(x[0][0], x[0][1], x[0][2], x[0][3], x[0][4], x[1], [], x[0][5], x[0][6], [], x[0][7], x[0][8], start);
	})
}
else
{	//個人モード
	...
	.then((x)=>{
		print_result(x[0][0], x[0][1], x[0][2], x[0][3], x[0][4], x[1], x[2], x[0][5], x[0][6], x[3], x[0][7], x[0][8], start);
